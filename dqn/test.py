...
tf.app.flags.DEFINE_integer('refresh_target_model_num', 25,  # update the target Q model every refresh_target_model_num
                            """frames over which to anneal epsilon.""")
tf.app.flags.DEFINE_integer('replay_memory', 20000,  # takes up to ? GB to store this amount of history data
...
def train_memory_batch(pmemory, nmemory, mmemory, model, log_dir):
    batch_size = FLAGS.batch_size//3

    pmini_batch = random.sample(pmemory, batch_size)
    nmini_batch = random.sample(nmemory, batch_size)
    mmini_batch = random.sample(mmemory, batch_size)
    mini_batch = pmini_batch + nmini_batch + mmini_batch
...
def train():
    ...
    pmemory = deque(maxlen=FLAGS.replay_memory)
    nmemory = deque(maxlen=FLAGS.replay_memory)
    mmemory = deque(maxlen=FLAGS.replay_memory)
    episode_number = 0
    ...
    while episode_number < FLAGS.num_episode:
        ...
        while not done:
            ...
            if dead:           
                store_memory(nmemory, history, action, reward, next_history, dead)
            elif reward > 0:
                store_memory(pmemory, history, action, reward, next_history, dead)
            else:
                store_memory(mmemory, history, action, reward, next_history, dead)
            ...
            # check if the memory is ready for training
                if global_step > FLAGS.observe_step_num:
                    loss = loss + train_memory_batch(pmemory, nmemory, mmemory, model, log_dir)
                    # if loss > 100.0:
                    #    print(loss)
                    if episode_number % FLAGS.refresh_target_model_num == 0:  # update the target model
                        model_target.set_weights(model.get_weights())
                ...
                if global_step <= FLAGS.observe_step_num:
                    ...
                    state = "train"
                pmemory_size = len(pmemory)
                nmemory_size = len(nmemory)
                mmemory_size = len(mmemory)
                print('state: {}, episode: {}, score: {}, global_step: {}, avg loss: {}, step: {}, pmemory length: {}, nmemory lenght: {}, mmemory lenght: {}'
                      .format(state, episode_number, score, global_step, loss / float(step), step, pmemory_size, nmemory_size, mmemory_size))
